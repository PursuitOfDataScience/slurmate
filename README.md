...
what `--force` is for, and it downgrades the check to a warning. The default just
is not silent.
...
An agent landing on a cluster it does not know can read the whole site, the
hardware in it, and your queue, then check a script before you ever see it.
Four read-only commands, all with `--json`. None of them submits anything.
...
`brief` replaces the half-dozen `sinfo` / `scontrol` / `sacctmgr` / `squeue`
calls an agent would otherwise guess its way through, and adds what those do not
say: whether a partition's CPU and memory figures are floors or ceilings, how
deep its queue is, which partitions your accounts can actually submit to, and
whether a GPU model can be requested by `--gres` type or only through
`--constraint`. A top-level `"mock": true` means no cluster was reachable, so
nothing under it is real.

```console
$ slurmate brief
  midway3 (Slurm 20.11.8, select/cons_tres, on midway3-login4)
  you: youzhi   accounts: rcc-staff, beagle3-users, ssd (+31 more)

  partition            nodes   cores     memory  time             queued  gpus
  caslake*           190/190      48     180 GB  infinite     2386r/575p
  gpu                  10/11     48+    180+ GB  infinite        36r/37p  yes
  amd                  39/40     128     244 GB  infinite        93r/78p
  beagle3              42/44      32     250 GB  infinite       86r/142p  yes
  (67 more none of your accounts can submit to; --all-partitions to see them)
```

`check` runs every validator in the package plus `sbatch --test-only`, which
enters no queue and spends no allocation, so it is safe in a loop. It reads
`module load` and `conda activate` out of the script body, not just the
`#SBATCH` block:

```console
$ slurmate check --script train.sbatch
  ✗ GPU type 'a100' is a node feature on 'gpu', not a GRES type, so gpu_format
  ...
  ✗ module 'cuda/12.1' not found on this cluster; 'cuda' is available as:
    cuda/10.2, cuda/11.2, cuda/11.3, cuda/11.5, cuda/11.7, ... (+12 more)
  ⚠ log directory '/tmp' is on node-local storage; the job still reports COMPLETED
  ✗ Slurm would refuse this job: Requested node configuration is not available
```

A check that cannot run says so rather than passing quietly, so "no problem
found" never has to be read as "I did not look".

Give `--script -` to read from stdin, or describe the job in flags instead
(`slurmate check -p gpu -c 8 --mem 32G -t 04:00:00`) to check a request before
anything is written.

`slurmate nodes` is the one `brief` cannot replace. A partition row aggregates
its nodes, which hides that one partition spans two GPU generations, so this
groups by what a job can actually ask for:
...
And `slurmate jobs` answers the question that follows a first submit. Every
pending job carries Slurm's own reason code plus what it means to do next,
because the codes are not interchangeable: `Priority` clears on its own,
`PartitionTimeLimit` never will.

### Three ways to wire it in

```bash
slurmate skill --install                    # .claude/skills/slurmate/SKILL.md
slurmate skill --install --format agents    # AGENTS.md, for Cursor/Codex/Copilot
slurmate skill --install --format mcp       # .mcp.json, registering the MCP server
```

The MCP route is the one that is a protocol rather than a document:
`slurmate mcp` serves the same four commands as typed tools over stdio, so a client
enumerates them instead of having to notice a markdown file. It needs no extra
dependency, and none of the tools can submit a job.

All three carry the same content, generated from the same file.
